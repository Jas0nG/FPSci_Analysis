...
class Trial:
    ...
    def show(self):
        ...
        print('tastId',self.tastId)
        print('tastIdx',self.tastIdx)
        print('startTime',self.startTime)
        print('endTime',self.endTime)
        print('duaration',self.duaration)
        print('taskExecTime',self.taskExecTime)
        print('destroyedTargets',self.destroyedTargets)
        print('totalTargets',self.totalTargets)
...
class Importer:
    ...
    def getTrials(self):
        ...
        for row in self.getTableRows('Trials'):
            ...
            trials.append(Trial(row[0], row[1], row[2], row[3], row[4], row[5], row[6],  row[7], row[8], row[9], row[10], row[11], tidx[row[0]]))
        return trials
    ...
    def getTrialTargetPositionsXYZ(self, trial, targetId=None):
        ...
        for row in self.queryDb(query): 
            if row[1] not in positions.keys(): positions[row[1]] = [[row[3], row[4], row[5]]]
            else: positions[row[1]].append([row[3], row[4], row[5]])
        return positions
    ...
    def getTrialPlayerActions(self, trial):
        ...
        for row in self.queryDb("SELECT * FROM Player_Action WHERE [time] <= \'" + trial.endTime + "\' AND [time] >= \'" + trial.startTime + "\'"): 
            actions.append(PlayerAction(row[0], row[1], row[2], row[3], row[4] , row[5], row[6], row[7], row[8]))
        return actions
    ...
    def getFrameInfo(self):
        ...
        for row in self.getTableRows('Frame_Info'):
            frames.append(FrameInfo(row[0], row[1]))
        return frames
