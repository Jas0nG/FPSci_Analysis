...
trials = db.getTrials()
clicks = db.queryDb('SELECT * from Player_Action WHERE [event] = \'hit\' OR [event] = \'miss\'')
...
score = []
for trial in trials:
    # query_Traget_Traj = "SELECT * FROM Target_Trajectory WHERE [time] <= \'{0}\' AND [time] >= \'{1}\'".format(trial.endTime, trial.startTime)
    query_Target = "SELECT * FROM Targets WHERE [spawn_time] <= \'{0}\' AND [spawn_time] >= \'{1}\'".format(trial.endTime, trial.startTime)
    #JasonG: Get the target information
    target = Targets(*db.queryDb(query_Target)[0])
    if(target.type == 'warm-up'):
        continue
    query_Traget_Traj = "SELECT * FROM Target_Trajectory WHERE [target_id] == \'{0}\'".format(target.id)
    ...
    # print('====Trial:', trial.index, 'Uses:', float(trial.taskExecTime) * 1000, 'ms====')
    ...
    query_destory = "SELECT * FROM Player_Action WHERE [time] <= \'{0}\' AND [time] >= \'{1}\'  AND [event] == 'destroy'".format(trial.endTime, trial.startTime)
    query_start_aim = "SELECT * FROM Player_Action WHERE [time] <= \'{0}\' AND [time] >= \'{1}\'  AND [event] == 'aim'".format(trial.endTime, trial.startTime)
    ...
    angle = polarAngle(action_start_aim.view_az, action_start_aim.view_el, action_destory.view_az, action_destory.view_el)
    # Lower the better
